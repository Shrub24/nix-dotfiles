-- Fork of Noctalia's community neovim template: base0C/0D/0E point at the bright
-- tonal tier instead of `*_fixed_dim`. Material You renders `*_fixed_dim` as the
-- same value as the base role, so upstream's mapping makes functions identical
-- to strings, keywords to classes and regex to constants.
--
-- Rendered by Noctalia (matugen) to $XDG_CONFIG_HOME/nvim/lua/matugen.lua.
-- Groups derived from it live in lua/config/theme.lua, which also owns the
-- SIGUSR1 refresh; upstream's signal handler is left out so there is one path.
 local M = {}
...
function M.setup()
  require('base16-colorscheme').setup({
    ...
    base0C = '{{colors.on_error_container.default.hex}}',

...
### Requirement: Lock screen authenticates through a dedicated PAM policy

The system SHALL patch Noctalia's lock screen to authenticate against a dedicated `noctalia-lock` PAM service consisting only of `pam_unix` auth and account stacks, leaving the system `login` policy unmodified.

#### Scenario: Lock screen unlock

- **WHEN** the session is locked and the user enters their password
- **THEN** authentication succeeds through `/etc/pam.d/noctalia-lock` without modifying `/etc/pam.d/login`

...
2. Replace `tokf` with `snip` in `programs.agentTools`.
3. Add `opencode-snip@latest` to the OpenCode plugin list.
4. Update architecture/structure docs and remove `pkgs/tokf`.
5. Verify `nix eval` and `nix build` for the Home Manager activation package.
6. If the build or plugin wiring regresses, roll back by restoring the `tokf` overlay entry/package and removing the `opencode-snip` plugin entry.

## Open Questions

1. Should `snip` eventually be switched back to nixpkgs once the packaged version catches up?
2. Do we want a follow-up to package `opencode-snip` declaratively, or is the imperative plugin model good enough long-term?

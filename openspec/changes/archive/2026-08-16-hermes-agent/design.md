...
1. Creates directories → `home.file` or `home.activation`
1. Writes config.yaml → `home.file.".hermes/config.yaml"`
1. Writes .env → `home.file.".hermes/.env"` or sops-nix
1. Systemd service → `systemd.user.services.hermes-agent`
1. CLI to PATH → `home.packages`
...
1. Should the gateway service be enabled by default or opt-in?

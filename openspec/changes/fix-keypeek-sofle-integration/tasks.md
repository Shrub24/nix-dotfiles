...
- [ ] 1.2 Apply a local source patch that classifies ZMK's default `1D50:615E` Raw-HID identity as ZMK before QMK fallback.
...
- [ ] 2.1 Build the patched Keypeek package and verify the desktop entry, icon, and patch are present in its output.
  - verify: `nix build .#homeConfigurations.saurabhj.activationPackage`
- [ ] 2.2 Switch Home Manager and confirm the Sofle follows ZMK discovery without a `keyboard_info.json` prompt.
